...
export default withAuth(
  async function middleware(request: NextRequest) {
    ...
    const pathnameIsMissingLocale = i18n.locales.every(locale => !pathname.startsWith(`/${locale}`))
    ...
    // if user not logged in and try to acess protected route
    ...
    if (isAuthPage && isAuth) {
      ...
      return NextResponse.redirect(new URL(`/${currentLocale}/${Routes.PROFILE}`, request.url))
    }
